...
typedef std::map<UID,PhantomNode> PhantomNodes;   ///< Type definiton for PahntomNodes UID is nid (internal node ID, NOT USER node Id).
...
#ifdef OSRMCLIENT
  ///< PahntomNodes information for pickups. UID is the id of pickups nodes.
  PhantomNodes mPhantomNodes;
  ///< Id for PahntomNodes as Node
  UID mIPNId = 900000;
  ///< Id for PahntomNodes as Node
  UID mPNId = 900000;
...
void process_pair_onPath(UINT i, UINT j) const {
      ...
      DLOG(INFO) << "(i,j)=("<< i << "," << j <<")";
      DLOG(INFO) << "travel_time_onTrip.size(): " << travel_time_onTrip.size();
  #endif
  ...
  if (travel_time_onTrip[i][j] == 0) {
    ...
    if (nodesOnPath.size() == 0 || nodesOnPath[0].nid() != original[i].nid()) {
      nodesOnPath.push_front(original[i]);
    }

    if (nodesOnPath[nodesOnPath.size()-1].nid() != original[j].nid()) {
      nodesOnPath.push_back(original[j]);
    }
  ...
  while ( git != geometry.end() ) {
    ...
    for ( unsigned int i=0; i<streetNodes.size(); i++ ) {
      ...
      if ( pos > 0 ) {
        ...
        osrmi->useOsrm(oldStateOsrm);
        // Add pickup if is in one_way or is right in the path. This give us best results!
...
private:
  ...
  void prepareTravelTimeonTrip() {

    #ifdef VRPMINTRACE
      DLOG(INFO) << "strarted prepareTravelTimeonTrip";
    #endif
...
private:
  ...
  double twc_for_ij(const knode &ni, const knode &nj) const {
    ...
    if ( (nj.closes() - ajei(ni, nj)) > 0 ) {
      result = std::min(ajli(ni, nj) , nj.closes()) - std::max(ajei(ni, nj) , nj.opens());
    } else {
      result = VRP_MIN();
    }
  ...
  void twcij_calculate() {
    #ifdef VRPMINTRACE
        DLOG(INFO) << "started twcij_calculate";
    #endif
    ...
    #ifdef VRPMINTRACE
        DLOG(INFO) << "ended twcij_calculate";
    #endif
  }
